<?php
// Certifique-se de que $post já foi buscado no banco de dados e está disponível.
if (!isset($post) || !$post) {
...
// Fetch Previous Post
$stmt_prev = $pdo->prepare("SELECT id, title, slug FROM posts WHERE id < ? AND active = 1 AND (status='publicado' OR status IS NULL) AND created_at <= NOW() ORDER BY id DESC LIMIT 1");
$stmt_prev->execute([$id]);
$prev_post = $stmt_prev->fetch();

// Fetch Next Post
$stmt_next = $pdo->prepare("SELECT id, title, slug FROM posts WHERE id > ? AND active = 1 AND (status='publicado' OR status IS NULL) AND created_at <= NOW() ORDER BY id ASC LIMIT 1");
$stmt_next->execute([$id]);
$next_post = $stmt_next->fetch();

$content_clean = preg_replace(['/<style\b[^>]*>(.*?)<\/style>/is', '/<script\b[^>]*>(.*?)<\/script>/is'], '', $post['content']);
$resumo_fallback = mb_substr(trim(preg_replace('/\s+/', ' ', strip_tags($content_clean))), 0, 150) . '...';

$dynamic_title = !empty($post['meta_title']) ? $post['meta_title'] : $post['title'];
$dynamic_desc = !empty($post['meta_description']) ? $post['meta_description'] : $resumo_fallback;
...
<div class="container section-padding" style="margin-top: 5rem; padding-left: 0; padding-right: 0;">
    <article class="blog-post-card">
        ...
        <div class="post-navigation">
            <?php if($prev_post): ?>
                <a href="/blog/<?= !empty($prev_post['slug']) ? $prev_post['slug'] : $prev_post['id'] ?>" class="nav-link prev">
            ...
            <?php if($next_post): ?>
                <a href="/blog/<?= !empty($next_post['slug']) ? $next_post['slug'] : $next_post['id'] ?>" class="nav-link next">
